...
int  main(int argc, char *argv[]) {
  ...
  if(!(quiet_mode||graph_mode)) {
    ...
    if(!collect_a_sample(s, interfaces_only)) {
      move(0,0);
      clrtobot();
	...
	if(last.stats.count(label) > 0) {
	  ...
	  if(bitrate > 0) {
	    render_line(linenum++,
			...
			"ext in", 
			dev.c_str(), 
			nowrate/traf_config[dev].interface_speed("download"), 
	...
	if(last.stats.count(label) > 0) {
	  ...
	  if(bitrate > 0) {
	    render_line(linenum++,
			...
			"ext out", 
			dev.c_str(), 
			nowrate/traf_config[dev].interface_speed("upload"), 
			format_rate(bitrate, nowrate).c_str(), 
			COLOR_BLUE);
	  }
	}
	...
	for (devidx = last.int_devs.begin(); devidx != last.ext_devs.end(); devidx++) {
      ...
      // lastly rules on a WHITE background
      if(show_rules) {
	...
	if(collect_a_sample(s, interfaces_only)) {
	  ...
	  for (devidx = int_devs.begin(); devidx != int_devs.end(); devidx++) { 
	   ...
	   // rules & classes only there if traffic running
	   ...
	   out += "\n" + itostr(last_sec.t.tv_sec) + ":" + itostr(last_sec.t.tv_usec) + "\n";
