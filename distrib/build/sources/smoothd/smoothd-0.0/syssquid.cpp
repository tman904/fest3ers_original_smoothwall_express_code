...
int start_squid( std::vector<std::string> & parameters, std::string & response )
{
	...
	if(clearedcache) 
	{
        	// syslog( LOG_WARNING, "Clearing squid cache" );
	...
	if(enable)
	{ 
        	// syslog( LOG_WARNING, "Starting squid" );
	...
	{
        	// syslog( LOG_WARNING, "Setting redirect to squid" );
		ipbatch("iptables -t nat -A squid -p tcp --dport 80 -j REDIRECT --to-ports 800");
		error = ipbatch("commit");
		if(error)
			response = "ipbatch failure";
	}
        // syslog( LOG_WARNING, "Changing ownership of  squid cache" );
	simplesecuresysteml("/bin/chown", "squid:squid",  "/var/spool/squid/cache", NULL);
