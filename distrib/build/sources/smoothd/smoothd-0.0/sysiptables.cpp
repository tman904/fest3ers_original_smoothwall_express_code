...
int load_portlist()
{
	...
	{
		syslog( LOG_ERR, "trying file %s", globbuf.gl_pathv[i] );
...
int  set_inbound( std::vector<std::string> & parameters, std::string & response )
{
		...
		line = fwdfile.next())
	{
		...
		if(enabled == "on")
		{
			...
			ipb.push_back("-t nat -A portfw -p " + protocol +
				" -s " + extip +  
				" -d " + localip.str() +  			
		...
		if((n = port.find_first_not_of(NUMBERS)) != std::string::npos)
		{
			if ( portlist[ port.c_str() ].size() > 0 ){
				...
				if(enabled == "on")
				{
					...
					for ( 
						std::vector<std::string>::iterator i = vect.begin();
						i != vect.end();
						i++
					    ){
						std::string nport = *i;
						syslog( LOG_ERR, "[m] iptables -A out%s -p tcp --dport %s -j ACCEPT", colour.c_str(), nport.c_str() );
			...
			syslog( LOG_ERR, "iptables -A out%s -p tcp --dport %s -j ACCEPT", colour.c_str(), port.c_str() );
