...
if ($settings{MSGONBATTERY} eq 'on') {
...
if ($settings{MSGOFFBATTERY} eq 'on') {
...
if ($settings{MSGCOMMFAILURE} eq 'on') {
...
if ($settings{MSGBATTATTACH} eq 'on') {
...
if ($settings{MSGBATTDETACH} eq 'on') {
...
if ($settings{MSGDOSHUTDOWN} eq 'on') {
...
if ($settings{MSGEMERGENCY} eq 'on') {
...
if ($settings{MSGENDSELFTEST} eq 'on') {
...
if ($settings{MSGKILLPOWER} eq 'on') {
...
if ($settings{MSGLOADLIMIT} eq 'on') {
...
if ($settings{MSGPOWERBACK} eq 'on') {
...
if ($settings{MSGREMOTEDOWN} eq 'on') {
...
if ($settings{MSGSTARTSELFTEST} eq 'on') {
...
system("chmod -R 0755 /var/smoothwall/apcupsd/scripts/");
